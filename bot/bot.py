...
⚪ /retry – Регенерировать последний ответ бота
...
🎨 Генерировать изображения по текстовым подсказкам в режиме <b>👩‍🎨 Художник</b> /mode.
...
Сделайте его <b>администратором</b>, чтобы он мог видеть сообщения (все остальные права можно ограничить).
Вы молодец!
...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = "Привет! Я - чат-бот <b>ChatGPT</b>, реализованный с использованием API OpenAI 🤖\n\n"
...
async def retry_handle(update: Update, context: CallbackContext):
    ...
    if len(dialog_messages) == 0:
        await update.message.reply_text("Нет сообщения для повторной отправки 🤷‍♂️")
        return
    ...
    async def message_handle_fn():
        ...
        if use_new_dialog_timeout:
            if (datetime.now() - db.get_user_attribute(user_id, "last_interaction")).seconds > config.new_dialog_timeout and len(db.get_dialog_messages(user_id)) > 0:
                db.start_new_dialog(user_id)
                await update.message.reply_text(f"Начало нового диалога из-за истечения времени ожидания.(<b>{config.chat_modes[chat_mode]['name']}</b> mode) ✅", parse_mode=ParseMode.HTML)
        db.set_user_attribute(user_id, "last_interaction", datetime.now())
        ...
        if n_first_dialog_messages_removed > 0:
            if n_first_dialog_messages_removed == 1:
                ...
                text = f"✍️ <i>Примечание:</i> Ваш текущий диалог слишком длинный, поэтому было удалено <b>{n_first_dialog_messages_removed} первых сообщений</b> из контекста. Отправьте команду /new, чтобы начать новый диалог."
            await update.message.reply_text(text, parse_mode=ParseMode.HTML)
...
async def generate_image_handle(update: Update, context: CallbackContext, message=None):
    ...
    try:
        ...
        if str(e).startswith("Your request was rejected as a result of our safety system"):
            text = "🥲 Ваш запрос <b>не соответствует</b> политике использования OpenAI. Что вы там написали, а?"
            await update.message.reply_text(text, parse_mode=ParseMode.HTML)
            return
        else:
            raise
...
async def show_balance_handle(update: Update, context: CallbackContext):
    ...
    if n_transcribed_seconds != 0:
        details_text += f"- Whisper (распознавание речи): <b>{voice_recognition_n_spent_dollars:.03f}$</b> / <b>{n_transcribed_seconds:.01f} секунд</b>\n"
...
async def post_init(application: Application):
    await application.bot.set_my_commands([
        ...
        BotCommand("/retry", "Регенерировать последний ответ бота"),
